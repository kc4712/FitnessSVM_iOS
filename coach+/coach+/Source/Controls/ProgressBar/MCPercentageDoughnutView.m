...
- (void)setDefaults
{
    ...
    self.adjustsFontSizeAutomatically   = NO;
    ...
    self.fontPercentText = [UIFont systemFontOfSize: 10];
    self.fontPercentMark = [UIFont systemFontOfSize: 10];
}
...
- (void)didDrawSublayers
{
    ...
    {
        self.fontPercentText = [UIFont systemFontOfSize:self.centerView.frame.size.width * 0.4];
        self.fontPercentMark = [UIFont systemFontOfSize:self.centerView.frame.size.width * 0.25];
    }

//    if (self.adjustsFontSizeAutomatically)
//    {
//        self.textLabel.font = self.fontPercentMark;
//    }
    self.textLabel.font = self.fontPercentMark;
    ...
    if (self.showTextLabel) {
        ...
        if (self.textStyle != MCPercentageDoughnutViewTextStyleUserDefined) {
            NSString *percent = [NSString stringWithFormat:@"%.*f", (int)self.decimalPlaces, self.percentage*100];
            NSString *txt = [NSString stringWithFormat:@"%@%%", percent];
            ...
            [text addAttributes:@{NSFontAttributeName : self.fontPercentText} range:NSMakeRange(0, percent.length)];
